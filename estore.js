...
/*
 ...
 Not using consolidate - waiting on better support for template inheritance with
...
MongoClient.connect('mongodb://localhost:27017/estore', function(err, db) {
    ...
    router.get("/item/:itemId", function(req, res) {
        ...
        items.getItem(itemId, function(item) {
            ...
            items.getRelatedItems(itemId, function(relatedItems) {
    ...
    router.post("/item/:itemId/reviews", function(req, res) {
        ...
        console.log('====> /item/:itemId/reviews')
        var itemId = parseInt(req.params.itemId);
     ...
     * interactions with the cart will be based on a single cart associated with
     * the the USERID constant we have defined above.
     *
     */
    ...
    router.post("/user/:userId/cart/items/:itemId", function(req, res) {
        ...
        console.log('====> /user/:userId/cart/items/:itemId')
        var userId = req.params.userId;
        var itemId = parseInt(req.params.itemId);
    ...
    router.post("/user/:userId/cart/items/:itemId/quantity", function(req, res) {
        ...
        console.log('====> /user/:userId/cart/items/:itemId/quantity')
        var userId = req.params.userId;
        var itemId = parseInt(req.params.itemId);
